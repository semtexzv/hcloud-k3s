...
resource null_resource k3s_server_installer {
  ...
  provisioner "remote-exec" {
    inline = [
      "INSTALL_K3S_VERSION=${local.k3s_version} INSTALL_K3S_EXEC=server sh /tmp/k3s-installer ${local.server_install_flags}",
      "until kubectl get nodes | grep -v '[WARN] No resources found'; do sleep 1; done",
      "scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@${lookup(var.server_node.connection, "host", var.server_node.ip)}:/etc/rancher/k3s/k3s.yaml ./config.yaml"
    ]
  }
}
